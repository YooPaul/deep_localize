...
class Map(object):
    def __init__(self, config_file, laser_max_range=10.0, downsample_factor=1):
        ...
        self.free_space = (bitmap >= 250).astype(np.uint8)

        #if downsample_factor > 1:
        #self.free_space = cv2.resize(self.free_space, None, fx=1.0/downsample_factor, fy=1.0/downsample_factor)
        self.free_space = cv2.resize(self.free_space, (512, 512))

        # 0 means no obstacle
        self.occupancy_grid = (1 - self.free_space) * 255
...
if __name__ == '__main__':
    ...
    for file in files:
        if file == '.DS_Store':
            continue
        f = (path + '/' + file)
        map_name = f + '/floorplan.yaml'
        m = Map(map_name, laser_max_range=10, downsample_factor=1)

        free_points = []
        for y in range(0,m.free_space.shape[0]): # 0 - 10
            for x in range(0,m.free_space.shape[1]):
                if m.free_space[y,x] == 1:
                    free_points.append((x,y))

        free_points = random.choices(free_points, k=10)

        degrees = [0, 45, 90, 135, 180, 225, 270, 315]
        degrees = random.choices(degrees, k=10)
        #degree = random.sample(degrees, 1)
        for i in range(10):
            x, y = m.world_coord(*free_points[i])
            degree = degrees[i]
            pos = np.array((x,y))
            heading = np.deg2rad(degree)
            n_ray = 100
            ...
            #xs, ys = m.grid_coord(obstacles[:, 0], obstacles[:, 1])
            xys = m.grid_coord_batch(obstacles)
            xys = np.clip(xys, 0, 511)

            new_m = m.free_space.copy()
            new_m[xys[:, 1], xys[:, 0]] = 2
            x,y = free_points[i]
            train_heading.append(degree)
            train_x.append(x)
            train_y.append(y)
            maps.append(new_m)
        counter += 1
    ...
    np.savez('dataset', maps=maps, xpos=train_x, ypos=train_y, heading=train_heading)
